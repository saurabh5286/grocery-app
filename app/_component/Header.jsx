...
function Header() {
  return (
    <div className='h-[80px] p-5 justify-between shadow-lg flex items-center'>
      <div className='flex items-center gap-8'>
        ...
        <Link href={"/category"} className='flex gap-2 items-center cursor-pointer border px-10 py-2 rounded-2xl bg-slate-300'>
  ...
  Category
      ...
      <div className='flex items-center gap-5'>
        ...
        <ShoppingBag />
        Checkout</Link>
        <Link href={"/login"} className='border px-8 py-2 rounded-lg font-bold *:**:'>Login</Link>
      </div>
    </div>
  )
}
